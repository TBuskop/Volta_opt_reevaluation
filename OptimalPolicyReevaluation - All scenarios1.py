...
experiments_slice2 = uncertainty_scenarios[int(2400/2):2400]
...
def main():
    ...
    with MultiprocessingEvaluator(model_set) as evaluator:
        results = evaluator.perform_experiments(scenarios1, n_policies)
        save_results(results, r'./reevaluation_results/results1.tar.gz')
    end = time.time()
